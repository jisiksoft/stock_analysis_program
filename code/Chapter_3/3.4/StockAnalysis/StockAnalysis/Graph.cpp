...
void CGraph::GetDataForChart(Company* company, PointData* ptData)
{
	...
	//------------------- Movement Average --------------------
	ptData->avg1.quantity = company->quantity - MOVE_AVERAGE_1;
	for(i=0; i<ptData->avg1.quantity; i++) {
		ptData->avg1.point[i].X = x_start - i * interval;
		ptData->avg1.point[i].Y = y_start - (HEIGHT_GRAPH * (company->moveAverage[i].avg1 - minVal)) / (maxVal - minVal);
	}
	...
	for(i=0; i<ptData->avg2.quantity; i++) {
		...
		ptData->avg2.point[i].Y = y_start - (HEIGHT_GRAPH * (company->moveAverage[i].avg2 - minVal)) / (maxVal - minVal);
	}
