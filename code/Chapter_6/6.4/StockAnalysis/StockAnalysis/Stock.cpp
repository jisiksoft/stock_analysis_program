...
{
	if (company->moveAverage.avg5[0] > company->moveAverage.avg20[0])
		return true;
...
{
	if (company->moveAverage.avg20[0] > company->moveAverage.avg60[0])
		return true;
...
void CStock::ReadDataFromFile()
{
	...
	for (int i=0; i<allCompanies.quantity; i++) {
		...
		allCompanies.companies[i].strJongMok.Format(_T("%s"), jongmok);
		allCompanies.companies[i].strName.Format(_T("%s"), name);
